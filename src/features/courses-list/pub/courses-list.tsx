import { revalidatePath } from 'next/cache';
import { coursesRepository } from '../courses.repository';
import { CourseItem } from '../ui/course-item';

export async function CoursesList({
  revalidatePagePath,
}: {
  revalidatePagePath: string; // путь для перепроверки страницы
}) {
  const coursesList = await coursesRepository.getCoursesList();

  const handleDeleteAction = async (courseId: string) => {
    'use server';

    await coursesRepository.deleteCourseElement({ id: courseId }); // вызов функции для удаления курса

    revalidatePath(revalidatePagePath); // перепроверка пути страницы
  };

  return (
    <div className="flex flex-col gap-3">
      {coursesList.map((course) => (
        <CourseItem
          key={course.id}
          course={course}
          onDelete={handleDeleteAction.bind(null, course.id)} // обработчик удаления курса, передаем null
        />
      ))}
    </div>
  );
}
